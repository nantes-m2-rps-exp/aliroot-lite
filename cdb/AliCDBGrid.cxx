...
AliCDBGrid::AliCDBGrid(const char *gridUrl, const char *user,
    ...
    : AliCDBStorage(), fGridUrl(gridUrl), fUser(user), fDBFolder(dbFolder),
      fSE(se), fMirrorSEs(""), fCacheFolder(cacheFolder),
      fOperateDisconnected(operateDisconnected), fCacheSize(cacheSize),
  ...
  if (!gGrid || fGridUrl != gGrid->GridUrl() ||
    ...
    if (gGrid) {
      AliInfo(Form("gGrid = %p; fGridUrl = %s; gGrid->GridUrl() = %s", gGrid,
                   fGridUrl.Data(), gGrid->GridUrl()));
      AliInfo(Form("fUser = %s; gGrid->GetUser() = %s", fUser.Data(),
                   gGrid->GetUser()));
    }
  ...
  if (fCacheFolder.Length() > 0) {
    if (origCache.Length() == 0) {
      ...
      AliWarning(
          Form("Local cache folder was already defined, changing it to: %s",
               fCacheFolder.Data()));
    }
    ...
    if (!TFile::ShrinkCacheFileDir(fCacheSize, fCleanupInterval)) {
      AliError(Form(
          "Could not set following values "
...
Bool_t AliCDBGrid::FilenameToId(TString &filename, AliCDBId &id) {
  ...
  if (filename.Contains(fDBFolder)) {
    filename =
        filename(fDBFolder.Length(), filename.Length() - fDBFolder.Length());
  }
...
Bool_t AliCDBGrid::IdToFilename(const AliCDBId &id, TString &filename) const {
  ...
  if (!id.GetAliCDBRunRange().IsValid()) {
    AliDebug(2, Form("Invalid run range <%d, %d>.", id.GetFirstRun(),
                     id.GetLastRun()));
    return kFALSE;
  }
  ...
  filename =
      Form("Run%d_%d_v%d", id.GetFirstRun(), id.GetLastRun(), id.GetVersion());
...
void AliCDBGrid::SetRetry(Int_t nretry, Int_t initsec) {
  ...
  AliWarning("WARNING!!! You are changing the exponential retry times and "
             "delay: this function should be used by experts!");
  fNretry = nretry;
  fInitRetrySeconds = initsec;
  AliDebug(2, Form("fNretry = %d, fInitRetrySeconds = %d", fNretry,
                   fInitRetrySeconds));
}
...
Bool_t AliCDBGrid::PrepareId(AliCDBId &id) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (!dirExist) {
      ...
      if (i == 1) {
        ...
        //		AliError(Form("Unexpected: could not remove %s
        ...
        if (!AddTag(dirName, "CDB")) {
          AliError(Form("Could not tag folder %s !", dirName.Data()));
          if (!gGrid->Rmdir(dirName.Data())) {
            AliError(Form("Unexpected: could not remove %s directory!",
                          dirName.Data()));
          }
          return 0;
        }
        ...
        //		AliError(Form("Could not tag folder %s !",
        // dirName.Data())); 		if(!gGrid->Rmdir(dirName.Data())){
        //			AliError(Form("Unexpected: could not remove %s
        // directory!", dirName.Data()));
        //		}
        //		return 0;
  ...
  for (int i = 0; i < res->GetEntries(); i++) {
    ...
    if (!FilenameToId(filename, anId))
      continue;
    if (anId.GetAliCDBRunRange().Overlaps(id.GetAliCDBRunRange()) &&
        anId.GetVersion() > lastVersion) {
  ...
  // GRP entries with explicitly set version escape default incremental
  // versioning
  if (id.GetPath().Contains("GRP") && id.HasVersion() && lastVersion != 0) {
    if (GetForbidGRPOverride()) {
      AliDebug(5, Form("Entry %s won't be put in the destination OCDB",
                       id.ToString().Data()));
      return kFALSE;
    } else {
      AliInfo("ATTENTION: Interdiction to override GRP* objects was explicitly "
              "disabled");
    }
  }
  ...
  if (lastStorage.Contains(TString("new"), TString::kIgnoreCase) &&
    ...
    AliDebug(2, Form("A NEW object is being stored with version %d",
                     id.GetVersion()));
    AliDebug(2,
             Form("and it will hide previously stored object with version %d!",
                  id.GetVersion() - 1));
  }

  if (!lastRunRange.IsAnyRange() &&
      !(lastRunRange.IsEqual(&id.GetAliCDBRunRange())))
...
AliCDBId *AliCDBGrid::GetEntryId(const AliCDBId &queryId) {
  ...
  if (selectedId.GetFirstRun() == fRun &&
      fPathFilter.Comprises(selectedId.GetAliCDBPath()) &&
      fVersion == selectedId.GetVersion() && !fMetaDataFilter) {
    // look into list of valid files previously loaded with
    // AliCDBStorage::FillValidFileIds()
    AliDebug(2, Form("List of files valid for run %d was loaded. Looking there "
                     "for fileids valid for path %s!",
                     selectedId.GetFirstRun(), selectedId.GetPath().Data()));
    dataId = GetId(fValidFileIds, selectedId);

  } else {
    // List of files valid for reqested run was not loaded. Looking directly
    // into CDB
    AliDebug(
        2, Form("List of files valid for run %d and version %d was not loaded. "
                "Looking directly into CDB for fileids valid for path %s!",
                selectedId.GetFirstRun(), selectedId.GetVersion(),
                selectedId.GetPath().Data()));

    TString filter;
    MakeQueryFilter(selectedId.GetFirstRun(), selectedId.GetLastRun(), 0,
                    filter);

    TString pattern = "Run*.root";
    TString optionQuery = "-y";
    if (selectedId.GetVersion() >= 0) {
      pattern.Insert(pattern.Index(".root"),
                     Form("_v%d_s0", selectedId.GetVersion()));
      optionQuery = "";
    }

    TString folderCopy(
        Form("%s%s/", fDBFolder.Data(), selectedId.GetPath().Data()));

    if (optionQuery.Contains("-y")) {
      AliInfo("Only latest version will be returned");
    }

    AliDebug(2, Form("** fDBFolder = %s, pattern = %s, filter = %s",
                     folderCopy.Data(), pattern.Data(), filter.Data()));
    TGridResult *res =
        gGrid->Query(folderCopy, pattern, filter, optionQuery.Data());
    if (res) {
      for (int i = 0; i < res->GetEntries(); i++) {
...
AliCDBEntry *AliCDBGrid::GetEntry(const AliCDBId &queryId) {
  ...
  TString errMessage(TString::Format(
      "No valid CDB object found! request was: %s", queryId.ToString().Data()));
...
AliCDBEntry *AliCDBGrid::GetEntryFromFile(TString &filename, AliCDBId *dataId) {
  ...
  if (fCacheFolder != "") {
    ...
    if (fCacheFolder != origCache) {
      AliWarning(Form("Local cache folder has been overwritten!! fCacheFolder "
                      "= %s origCache = %s",
  ...
  if (!anEntry) {
    AliDebug(
        2,
        Form("Bad storage data: file does not contain an AliCDBEntry object!"));
    file->Close();
    return NULL;
  }

  anEntry->PrintMetaData();

  // The object's Id is not reset during storage
  // If object's Id runRange or version do not match with filename,
  // it means that someone renamed file by hand. In this case a warning msg is
  // issued.

  if (anEntry) {
    AliCDBId entryId = anEntry->GetId();
    Int_t tmpSubVersion = dataId->GetSubVersion();
    dataId->SetSubVersion(
        entryId.GetSubVersion()); // otherwise filename and id may mismatch
...
TList *AliCDBGrid::GetEntries(const AliCDBId &queryId) {
  ...
  if (queryId.GetFirstRun() == fRun &&
    ...
    // look into list of valid files previously loaded with
    // AliCDBStorage::FillValidFileIds()
    AliDebug(2, Form("List of files valid for run %d and for path %s was "
                     "loaded. Looking there!",
    ...
    // List of files valid for reqested run was not loaded. Looking directly
    // into CDB
    AliDebug(2, Form("List of files valid for run %d and for path %s was not "
                     "loaded. Looking directly into CDB!",
                     queryId.GetFirstRun(), queryId.GetPath().Data()));
  ...
  // According to query and to the selection criteria list, version can be the
  // highest or exact
  AliCDBPath pathCopy;
  AliCDBId *anIdPtr = 0;
  AliCDBId *dataId = 0;
  ...
  while ((anIdPtr = dynamic_cast<AliCDBId *>(iter->Next()))) {
    ...
    if (!(queryPath.Comprises(thisCDBPath)) ||
        pathCopy.GetPath() == thisCDBPath.GetPath())
      continue;
...
Bool_t AliCDBGrid::PutEntry(AliCDBEntry *entry, const char *mirrors) {
  ...
  TString folderToTag = Form("%s%s", fDBFolder.Data(), id.GetPath().Data());
  ...
  // if a list of SEs was passed to this method or set via SetMirrorSEs, set the
  // first as SE for opening the file. The other SEs will be used in cascade in
  // case of failure in opening the file. The remaining SEs will be used to
  // create replicas.
  ...
  if (nSEs == 0) {
    if (fSE != "default")
      fullFilename += Form("?se=%s", fSE.Data());
  } else {
    remainingSEs = nSEs;
  }
  ...
  while (!reOpenResult &&
         reOpenAttempts <
             2) { // loop to check the file after closing it, to catch the
                  // unlikely but possible case when the file
    // is cleaned up by alien just before closing as a consequence of a network
    ...
    while (!file && remainingSEs > 0) {
      if (nSEs != 0) {
        ...
        if (!(targetSE.BeginsWith("ALICE::") && targetSE.CountChar(':') == 4)) {
          AliError(Form("\"%s\" is an invalid storage element identifier.",
                        targetSE.Data()));
          continue;
        }
      ...
      Int_t nsleep = fInitRetrySeconds; // number of seconds between attempts.
                                        // We let it increase exponentially
      AliDebug(2, Form("Uploading file into SE #%d: %s",
                       nSEs - remainingSEs + 1, targetSE.Data()));
      while (remainingAttempts > 0) {
        AliDebug(2, Form("Uploading file into OCDB at %s - Attempt #%d",
                         targetSE.Data(), fNretry - remainingAttempts + 1));
        remainingAttempts--;
        ...
        if (!file || !file->IsWritable()) {
          ...
          TString message(TString::Format("Attempt %d failed.",
    ...
    if (!file) {
      AliError(Form("All %d attempts have failed on all %d SEs. Returning...",
                    fNretry, nSEs));
      return kFALSE;
    }
    ...
    if (!result) {
      ...
      AliDebug(2, Form("Reopening file %s for checking its correctness",
                       fullFilename.Data()));
      reopenedFile = TFile::Open(fullFilename.Data(), "READ");
      if (!reopenedFile) {
        reOpenResult = kFALSE;
        AliInfo(
            Form("The file %s was closed successfully but cannot be reopened. "
                 "Trying now to regenerate it (regeneration attempt number %d)",
  ...
  // In case of other SEs specified by the user, mirror the file to the
  // remaining SEs
  ...
  if (AliCDBManager::Instance()
          ->IsOCDBUploadMode()) { // if uploading to OCDBs, add to cvmfs too
    if (!filename.BeginsWith("/alice/data") &&
        !filename.BeginsWith("/alice/simulation/2008/v4-15-Release")) {
      AliError(Form(
          "Cannot upload to CVMFS OCDBs a non official CDB object: \"%s\"!",
          filename.Data()));
    } else {
      if (!PutInCvmfs(filename, reopenedFile))
        AliError(Form("Could not upload AliEn file \"%s\" to CVMFS OCDB!",
                      filename.Data()));
    }
    reopenedFile->Close();
    delete reopenedFile;
...
Bool_t AliCDBGrid::PutInCvmfs(TString &filename, TFile *cdbFile) const {
  ...
  TString basename =
      (cvmfsFilename(cvmfsFilename.Last('/') + 1, cvmfsFilename.Length()));
  TString cvmfsDirname =
      cvmfsFilename.Remove(cvmfsFilename.Last('/'), cvmfsFilename.Length());
  ...
  AliDebug(3, Form("Publishing \"%s\" in \"%s\"", basename.Data(),
                   cvmfsDirname.Data()));

  // Tar the file with the right prefix path. Include the directory structure in
  // the tarball to cover the case of a containing directory being new in cvmfs,
  // plus a container directory to avoid clashing with stuff present in the
  // local directory
  TString firstLevel(threeLevels(0, threeLevels.First('/')));
  TString tempDir("tmpToCvmfsOcdbs");
  gSystem->Exec(
      Form("rm -r %s > /dev/null 2>&1",
           tempDir.Data())); // to be sure not to publish other stuff in cvmfs
  Int_t result =
      gSystem->Exec(Form("mkdir -p %s/%s", tempDir.Data(), threeLevels.Data()));
  if (result != 0) {
    AliError(Form("Could not create the directory \"%s/%s\"", tempDir.Data(),
                  threeLevels.Data()));
    return kFALSE;
  }
  cdbFile->Cp(
      Form("%s/%s/%s", tempDir.Data(), threeLevels.Data(), basename.Data()));
  ...
  // 's,^,/cvmfs/alice-ocdb.cern.ch/calibration/data/2010/OCDB/,S' -cvzf
  // objecttoadd.tar.gz basename
  result = gSystem->Exec(Form("tar --transform 's,^%s,%s,S' -cvzf %s %s",
                              tempDir.Data(), cvmfsBaseFolder.Data(),
                              tarFileName.Data(), tempDir.Data()));
  if (result != 0) {
    AliError(Form("Could not create the tarball for the object \"%s\"",
                  filename.Data()));
    return kFALSE;
  }

  // Copy the file to cvmfs (requires to have the executable in the path and
...
Bool_t AliCDBGrid::AddTag(TString &folderToTag, const char *tagname) {
  ...
  if (resCode[0] != '1') {
    AliError(Form("Couldn't add %s tags to folder %s !", tagname,
                  folderToTag.Data()));
    result = kFALSE;
  }
  delete gridres;
...
Bool_t AliCDBGrid::TagFileId(TString &filename, const AliCDBId *id) {
  ...
  TString addTagValue2 =
      Form("first_run=%d last_run=%d version=%d ", id->GetFirstRun(),
           id->GetLastRun(), id->GetVersion());
  TString addTagValue3 =
      Form("path_level_0=\"%s\" path_level_1=\"%s\" path_level_2=\"%s\" ",
           id->GetPathLevel(0).Data(), id->GetPathLevel(1).Data(),
  ...
  TString addTagValue4 = Form("version_path=\"%09d%s\" dir_number=%d",
                              id->GetVersion(), filename.Data(), dirNumber);
  TString addTagValue =
      Form("%s%s%s%s", addTagValue1.Data(), addTagValue2.Data(),
           addTagValue3.Data(), addTagValue4.Data());
  ...
  if (resCode[0] != '1') {
    AliError(Form("Couldn't add CDB tag value to file %s !", filename.Data()));
    result = kFALSE;
  } else {
...
Bool_t AliCDBGrid::TagShortLived(TString &filename, Bool_t value) {
  ...
  TString addTagValue =
      Form("addTagValue %s ShortLived_try value=%d", filename.Data(), value);
  ...
  const char *resCode = res->GetKey(0, "__result__"); // '1' if success
  if (resCode[0] != '1') {
    AliError(Form("Couldn't add ShortLived tag value to file %s !",
                  filename.Data()));
    result = kFALSE;
  } else {
    AliDebug(2, "Object successfully tagged.");
...
Bool_t AliCDBGrid::TagFileMetaData(TString &filename,
  ...
  TString addTagValue2 =
      Form("object_classname=\"%s\" responsible=\"%s\" beam_period=%d ",
           md->GetObjectClassName(), md->GetResponsible(), md->GetBeamPeriod());
  TString addTagValue3 = Form("aliroot_version=\"%s\" comment=\"%s\"",
                              md->GetAliRootVersion(), md->GetComment());
  TString addTagValue = Form("%s%s%s", addTagValue1.Data(), addTagValue2.Data(),
                             addTagValue3.Data());
  ...
  if (resCode[0] != '1') {
    AliWarning(
        Form("Couldn't add CDB_MD tag value to file %s !", filename.Data()));
    result = kFALSE;
  } else {
    AliDebug(2, "Object successfully tagged.");
    result = kTRUE;
...
void AliCDBGrid::QueryValidFiles() {
  ...
  if (fMaxDate) {
    ...
    AliInfoF("Returning only latest versions as they were at %s UTC",
             TTimeStamp(fMaxDate).AsString("s"));
  }
  if (fVersion >= 0) {
    ...
    if ((entry = (TObjString *)(map->GetValue("lfn")))) {
      ...
      if (result) {
        if (fMaxDate) {
          ...
          if (!o_guid)
            AliFatal("guid not found in AliEn find query result!");
...
void AliCDBGrid::MakeQueryFilter(Int_t firstRun, Int_t lastRun,
  ...
  //		result += Form(" and CDB:path_level_0=\"%s\"",
  // pathFilter.GetLevel0().Data());
  //	}
  //	if(pathFilter.GetLevel1() != "*") {
  //		result += Form(" and CDB:path_level_1=\"%s\"",
  // pathFilter.GetLevel1().Data());
  //	}
  //	if(pathFilter.GetLevel2() != "*") {
  //		result += Form(" and CDB:path_level_2=\"%s\"",
  // pathFilter.GetLevel2().Data());
  //	}

  if (md) {
    if (md->GetObjectClassName()[0] != '\0') {
      result +=
          Form(" and CDB_MD:object_classname=\"%s\"", md->GetObjectClassName());
    }
    ...
    if (md->GetAliRootVersion()[0] != '\0') {
      result +=
          Form(" and CDB_MD:aliroot_version=\"%s\"", md->GetAliRootVersion());
    }
...
AliCDBParam *AliCDBGridFactory::CreateParameter(const char *gridString) {
  ...
  AliDebug(2,
           Form("local cache operate disconnected: %d", operateDisconnected));
  AliDebug(2, Form("local cache size: %lld", cacheSize));
  AliDebug(2, Form("local cache cleanup interval: %ld", cleanupInterval));

  if (dbFolder == "") {
    AliError("The CDB base folder must be specified by preceding it with the "
             "\"folder=\" key!");
    AliError("Example: \"alien://folder=/alice/data/2015/OCDB\"");
    return NULL;
  }

  return new AliCDBGridParam(gridUrl.Data(), user.Data(), dbFolder.Data(),
                             se.Data(), cacheFolder.Data(), operateDisconnected,
...
AliCDBStorage *AliCDBGridFactory::Create(const AliCDBParam *param) {
  ...
  if (AliCDBGridParam::Class() == param->IsA()) {
    ...
    grid = new AliCDBGrid(
        gridParam->GridUrl().Data(), gridParam->GetUser().Data(),
        gridParam->GetDBFolder().Data(), gridParam->GetSE().Data(),
        gridParam->GetCacheFolder().Data(), gridParam->GetOperateDisconnected(),
...
ClassImp(AliCDBGridParam)
    ...
    : AliCDBParam(), fGridUrl(), fUser(), fDBFolder(), fSE(), fCacheFolder(),
...
//_____________________________________________________________________________
AliCDBGridParam::AliCDBGridParam(const char *gridUrl, const char *user,
                                 const char *dbFolder, const char *se,
                                 const char *cacheFolder,
                                 Bool_t operateDisconnected, Long64_t cacheSize,
                                 Long_t cleanupInterval)
    : AliCDBParam(), fGridUrl(gridUrl), fUser(user), fDBFolder(dbFolder),
      fSE(se), fCacheFolder(cacheFolder),
      fOperateDisconnected(operateDisconnected), fCacheSize(cacheSize),
      fCleanupInterval(cleanupInterval) {
  // constructor
  ...
  TString uri = Form(
      ...
      fGridUrl.Data(), fUser.Data(), fDBFolder.Data(), fSE.Data(),
      fCacheFolder.Data(), fOperateDisconnected, fCacheSize, fCleanupInterval);
...
Bool_t AliCDBGridParam::IsEqual(const TObject *obj) const {
  ...
  if (fGridUrl != other->fGridUrl)
    return kFALSE;
  if (fUser != other->fUser)
    return kFALSE;
  if (fDBFolder != other->fDBFolder)
    return kFALSE;
  if (fSE != other->fSE)
    return kFALSE;
  if (fCacheFolder != other->fCacheFolder)
    return kFALSE;
  if (fOperateDisconnected != other->fOperateDisconnected)
    return kFALSE;
  if (fCacheSize != other->fCacheSize)
    return kFALSE;
  if (fCleanupInterval != other->fCleanupInterval)
    return kFALSE;
  return kTRUE;
}
