...
AliDCSSensor::AliDCSSensor()
    : fId(), fIdDCS(0), fStringID(), fStartTime(0), fEndTime(0), fGraph(0),
      fFit(0), fX(0), fY(0), fZ(0) {
  //
...
AliDCSSensor::AliDCSSensor(const AliDCSSensor &source)
    : TNamed(source), fId(source.fId), fIdDCS(source.fIdDCS),
      fStringID(source.fStringID), fStartTime(source.fStartTime),
      fEndTime(source.fEndTime), fGraph(0), fFit(0), fX(source.fX),
      fY(source.fY), fZ(source.fZ)
//
//  Copy constructor
//
{
  if (source.fGraph)
    fGraph = (TGraph *)source.fGraph->Clone();
  if (source.fFit)
    fFit = (AliSplineFit *)source.fFit->Clone();
}
...
void AliDCSSensor::Draw(Option_t *option) {
  ...
  // Unfortuantelly - it  make a memory leak as function Draw does not return
  // the object pointer
  //
  // TCanvas * canvas = new TCanvas((fStringID+option).Data(),
  // (fStringID+option).Data());
  if (fGraph) {
  ...
  TGraph *graph = MakeGraph(
      100); // memory leak - we can not modify the content - const method
  if (graph) {
    graph->Draw(option); //
  }
}
...
TGraph *AliDCSSensor::MakeGraph(Int_t nPoints, Bool_t debug) const {
  ...
  if (debug == kTRUE) {
    printf("Start time %d, End time %d, step time %d\n", fStartTime, fEndTime,
           stepTime);
    TTimeStamp t((time_t)fStartTime, 0);
    t.Print();
    TTimeStamp t2((time_t)fEndTime, 0);
    t2.Print();
  }
